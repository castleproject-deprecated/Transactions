namespace Castle.NHibIntegration
{
	...
	public class DefaultConfigurationBuilder : IConfigurationBuilder
	{
		...
		{
			Factories = new NhFactoryConfiguration[0];
		}
		...
		/// <returns></returns>
		public virtual NHibernate.Cfg.Configuration GetConfiguration(IConfiguration config)
		{
		...
		/// <param name="cfg">The CFG.</param>
		/// <param name="facilityConfig">The facility config.</param>
		protected void RegisterListeners(NHibernate.Cfg.Configuration cfg, IConfiguration facilityConfig)
		{
			if (facilityConfig == null) return;
		...
		protected void GenerateMappingFromAttributesIfNeeded(NHibernate.Cfg.Configuration cfg, String targetAssembly)
		{
